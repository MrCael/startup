...
export function Purchase() {
    ...
    async function updateCart() {
        await fetch(`/api/purchase/${checkoutId}`, {
            method: "PATCH"
        });
    }

    useEffect(() => {
        async function getUserPurchaseInfo() {
            const response = await fetch("/api/purchase/info");
            const body = await response.json();

            setShippingInfo(body.addressList);
            setBillingInfo(body.cardList);
        }
            ...
            <div className="d-flex flex-column justify-content-center align-div">
                <table style={{marginBottom: "10px"}}>
                    <tbody>
                                ...
                                {shippingInfo.map((address, index) => {
                                    return (
                                        <div className="card" style={{ width: "fit-content", margin: "auto", marginTop: "10px", marginBottom: "10px" }}>
                                            <input type="radio" id={`shipping_${index}`} name="shipping" style={{ marginTop: "5px", float: "left" }} />
                        ...
                        <tr>
                            <td>
                                {billingInfo.map((card, index) => {
                                    return (
                                        <div className="card" style={{ width: "fit-content", margin: "auto", marginTop: "10px", marginBottom: "10px" }}>
                                            <input type="radio" id={`billing_${index}`} name="billing" style={{ marginTop: "5px" }} />
                                            <label for={`billing_${index}`}>
